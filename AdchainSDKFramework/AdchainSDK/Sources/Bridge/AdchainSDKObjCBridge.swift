...
public class AdchainSDKObjCBridge: NSObject {
    ...
    @objc public func login(userId: String,
        ...
        do {
            try AdchainSdk.shared.login(adchainSdkUser: user, listener: listener)
        } catch {
            completion(false, error.localizedDescription)
        }
    }
...
private class AdchainSdkLoginListenerWrapper: AdchainSdkLoginListener {
...
private class OfferwallCallbackWrapper: OfferwallCallback {
