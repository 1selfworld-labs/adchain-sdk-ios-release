...
protocol ApiService {
    ...
    func getBannerInfo(userId: String, placementId: String, platform: String) async throws -> BannerInfoResponse
}
...
class ApiServiceImpl: ApiService {
    ...
    func validateApp() async throws -> ValidateAppResponse {
        ...
        if let httpResponse = response as? HTTPURLResponse {
            AdchainLogger.v("ApiService", "=== ValidateApp Response Debug ===")
            AdchainLogger.v("ApiService", "Status Code: \(httpResponse.statusCode)")
            AdchainLogger.v("ApiService", "Headers: \(httpResponse.allHeaderFields)")
        }
        
        // Print raw JSON response
        if let jsonString = String(data: data, encoding: .utf8) {
            AdchainLogger.v("ApiService", "Raw JSON Response: \(jsonString)")
        }
        ...
        do {
            ...
            AdchainLogger.d("ApiService", "Successfully decoded response: \(validationResponse)")
            return validationResponse
        } catch {
            AdchainLogger.e("ApiService", "Decoding error: \(error)", error)
            
            // Try to parse as dictionary to see structure
            if let json = try? JSONSerialization.jsonObject(with: data, options: []) as? [String: Any] {
                AdchainLogger.v("ApiService", "Response as Dictionary: \(json)")
                if let app = json["app"] as? [String: Any] {
                    AdchainLogger.v("ApiService", "App object contents: \(app)")
                }
            }
    ...
    func login(_ request: LoginRequest) async throws -> LoginResponse {
        ...
        // Debug logging
        if let httpResponse = response as? HTTPURLResponse {
            AdchainLogger.v("ApiService", "=== Login Response Debug ===")
            AdchainLogger.v("ApiService", "Status Code: \(httpResponse.statusCode)")
            AdchainLogger.v("ApiService", "Headers: \(httpResponse.allHeaderFields)")
        }
        ...
        do {
            ...
            AdchainLogger.d("ApiService", "Successfully decoded response: \(response)")
            return response
        } catch {
            AdchainLogger.e("ApiService", "Decoding error: \(error)", error)
    ...
    func getQuizEvents(userId: String? = nil, platform: String? = nil, ifa: String) async throws -> QuizResponse {
    ...
    func getMissions(userId: String? = nil, platform: String? = nil, ifa: String) async throws -> MissionResponse {
    ...
    func getBannerInfo(userId: String, placementId: String, platform: String) async throws -> BannerInfoResponse {
        var urlComponents = URLComponents(string: "\(baseURL)/v1/api/sdk/banner")!
        ...
        let (data, _) = try await session.data(for: urlRequest)
        let decoder = JSONDecoder()
        return try decoder.decode(BannerInfoResponse.self, from: data)
    }
}
